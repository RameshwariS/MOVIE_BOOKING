...
const createTheater = async (req, res) => {
  try {
    ...
    if (result.err) {
      return res.status(result.code).json({ err: result.err, data: {}, msg: "something went wrong", success: false });
    }
    return res.status(201).json({ err: {}, data: result, msg: "Theater created successfully", success: true });
  } catch (err) {
    console.error(err, 'in theater creation');
...
const getTheater = async (req, res) => {
  try {
    ...
    return res.status(200).json({ err: {}, data: result, msg: "Theater fetched successfully", success: true });
  } catch (err) {
    console.error(err, 'in theater get');
    return res.status(500).json({ err: err.message, data: {}, msg: "something went wrong", success: false });
  }
}
