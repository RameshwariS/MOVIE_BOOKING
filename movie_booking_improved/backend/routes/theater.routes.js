...
const { validateTheaterCreateRequest } = require('../middlewares/theater.middleware');

const routes = (app) => {
    app.post('/mba/api/v1/theaters', verifyToken, requireAdmin, validateTheaterCreateRequest, theaterController.createTheater);
    app.get('/mba/api/v1/theaters/:id', theaterController.getTheater);
    app.get('/mba/api/v1/theaters', theaterController.getTheaters);
    app.put('/mba/api/v1/theaters/:id', verifyToken, requireAdmin, validateTheaterCreateRequest, theaterController.updateTheater);
